...
public class Menu : MonoBehaviour
{
	...
	void Awake () 
	{
		if (!PlayerPrefs.HasKey("language"))
		{
			...
			actualLanguage = GameObject.Find("DATABASES").transform.FindChild("Words_Database").GetComponent<Word_Database>().languagesName[0];
	...
	public void Start () 
	{
		...
		instantiatedButtons = new GameObject[GameObject.Find("DATABASES").transform.FindChild("Words_Database").GetComponent<Word_Database>().languagesName.Length];
        GameObject.Find("MENU").transform.FindChild("LikeFacebookButton").transform.FindChild("Text").GetComponent<TextMesh>().text = "Like  +"+ coinsByLikeFb;
			...
			if (hit.collider != null) 
			{
				...
				else if (objectHit.name == "LanguagesContainer")
				{
					...
					if (!showLanguageButtons)
					{
						...
						{
                            if (GameObject.Find("DATABASES").transform.FindChild("Words_Database").GetComponent<Word_Database>().languagesName[i] != actualLanguage) 
							{
