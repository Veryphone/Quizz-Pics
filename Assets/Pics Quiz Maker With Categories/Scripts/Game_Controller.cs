...
public class Game_Controller : MonoBehaviour
{
	...
	void findObjectsOfVariables () 
	{
        ...
        wordDB = GameObject.Find ("DATABASES").transform.FindChild(RefreshQuizSelectionLevels.selectedCategory).GetComponent<Word_Database>();
	}
	...
	void setImageAndWord ()
	{
        ...
        if (RefreshQuizSelectionLevels.levelSelected != -1)
        {
           switch (PlayerPrefs.GetInt("numberLanguae"))
		    ...
		    GameObject.Find("GAME").transform.FindChild("Image").GetComponent<Renderer>().material.mainTexture = textureFromSprite(wordDB.image[RefreshQuizSelectionLevels.levelSelected]);
							...
							if (!fieldFounded)
							{
								...
								if (separedWord[i] != "/" && separedWord[i] != " ")
								{
									if(instanciedAnswerFields[i].transform.FindChild ("Letter").GetComponent<TextMesh>().text == "") 
									{
										...
										instanciedAnswerFields[i].transform.FindChild ("Letter").transform.Find("LetterShadow").GetComponent<TextMesh>().text = objectHit.gameObject.transform.FindChild("Letter").GetComponent<TextMesh>().text;
										objectHit.GetComponent<Collider2D>().enabled = false;
										objectHit.GetComponent<SpriteRenderer>().color = new Color(1f, 1f, 1f, 0f);
										objectHit.transform.FindChild("Letter").GetComponent<TextMesh>().color = new Color(1f, 1f, 1f, 0f);
										objectHit.transform.FindChild("Letter").transform.FindChild("LetterShadow").GetComponent<TextMesh>().color = new Color(1f, 1f, 1f, 0f);
										fieldFounded = true;
										lettersPressed += 1;
									}
								}
					...
					if (!win)
					{
						for (int h = 0; h < instanciedButtons.Length; h++) 
						{
							...
							if (!instanciedButtons[h].GetComponent<Collider2D>().enabled && instanciedButtons[h].transform.FindChild("Letter").GetComponent<TextMesh>().text == objectHit.transform.FindChild ("Letter").GetComponent<TextMesh>().text)
							{
								...
								objectHit.transform.FindChild ("Letter").FindChild("LetterShadow").GetComponent<TextMesh>().text = "";
								...
								instanciedButtons[h].transform.FindChild("Letter").GetComponent<TextMesh>().color = new Color(1f, 1f, 1f, 1f);
								instanciedButtons[h].transform.FindChild("Letter").transform.FindChild("LetterShadow").GetComponent<TextMesh>().color = new Color(0f, 0f, 0f, 0.4f);
								lettersPressed -= 1;
							}
						}
					}
                ...
                else if (objectHit.name == "HelpSolveWord")
                {
                    ...
                    if (!win)
                    {
                        ...
                        else
                        {
                            ...
                            Destroy(window.transform.FindChild("ButtonNo").gameObject);
                            Destroy(window.transform.FindChild("TextNo").gameObject);
                            window.transform.FindChild("ButtonOk").transform.position = new Vector3(0f, window.transform.FindChild("ButtonOk").transform.position.y, window.transform.FindChild("ButtonOk").transform.position.z);
                            window.transform.FindChild("TextOk").transform.position = new Vector3(0, window.transform.FindChild("TextOk").transform.position.y, window.transform.FindChild("TextOk").transform.position.z);
    ...
    void ShowCoinsLogs ()
	{
		...
		if (addCoinsLog)
		{
			...
			objInstantiated = Instantiate(particleCoins.gameObject, new Vector3 (coinsPlayerGame.transform.parent.transform.FindChild("Coin").transform.position.x, coinsPlayerGame.transform.parent.transform.FindChild("Coin").transform.position.y, -3.07f), transform.rotation) as GameObject;
				...
				cameraObj.GetComponent<AudioSource>().PlayOneShot(cameraObj.GetComponent<Sounds>().buyHelp);
						...
						for (int j = 0; j < word.Length; j++) 
						{
								...
								if (separedWord [j] != " " && separedWord [j] != "/") 
								{
										...
										instanciedAnswerFields [j].transform.FindChild ("Letter").FindChild("LetterShadow").GetComponent<TextMesh> ().text = separedWord [j];
			...
			if (PlayerPrefs.GetInt ("coinsPlayer") >= wordDB.coinsToShowOneLetter) 
			{
					...
					if (separedWord [randomNumber] != "/" && separedWord [randomNumber] != " ") 
					{
							...
							if (instanciedAnswerFields [randomNumber].transform.FindChild ("Letter").GetComponent<TextMesh> ().text == "") 
							{
									...
									instanciedAnswerFields [randomNumber].transform.FindChild ("Letter").FindChild("LetterShadow").GetComponent<TextMesh> ().text = separedWord [randomNumber];
			...
			for (int y = 0; y < word.Length; y++) 
			{
				if (!win)
				{
                    ...
                    if (playerAnswer == word)
                    {
                        ...
                        instantiateCongratulations.transform.FindChild("TextCoins").GetComponent<TextMesh>().text = "+" + wordDB.coinsWinedByWord;
                        if (!wordDB.showInfoLaterOfSolveWord)
                        {
                            Destroy(instantiateCongratulations.transform.FindChild("Canvas").gameObject);
                            Destroy(instantiateCongratulations, 5);

                        }
                        else
                        {
                            instantiateCongratulations.transform.FindChild("Canvas").transform.FindChild("ButtonNext").transform.FindChild("Text").GetComponent<TextMesh>().text = wordDB.uiTextsLang[PlayerPrefs.GetInt("numberLanguae"), 20];

                            switch (PlayerPrefs.GetInt("numberLanguae"))
                            {
                                case 0:
                                    instantiateCongratulations.transform.FindChild("Canvas").transform.FindChild("Text").GetComponent<Text>().text = wordDB.languageInfoWord0[RefreshQuizSelectionLevels.levelSelected];
                                    break;
                                case 1:
                                    instantiateCongratulations.transform.FindChild("Canvas").transform.FindChild("Text").GetComponent<Text>().text = wordDB.languageInfoWord1[RefreshQuizSelectionLevels.levelSelected];
                                    break;
                                case 2:
                                    instantiateCongratulations.transform.FindChild("Canvas").transform.FindChild("Text").GetComponent<Text>().text = wordDB.languageInfoWord2[RefreshQuizSelectionLevels.levelSelected];
                                    break;
                                case 3:
                                    instantiateCongratulations.transform.FindChild("Canvas").transform.FindChild("Text").GetComponent<Text>().text = wordDB.languageInfoWord3[RefreshQuizSelectionLevels.levelSelected];
                                    break;
                                case 4:
                                    instantiateCongratulations.transform.FindChild("Canvas").transform.FindChild("Text").GetComponent<Text>().text = wordDB.languageInfoWord4[RefreshQuizSelectionLevels.levelSelected];
                                    break;
                                case 5:
                                    instantiateCongratulations.transform.FindChild("Canvas").transform.FindChild("Text").GetComponent<Text>().text = wordDB.languageInfoWord5[RefreshQuizSelectionLevels.levelSelected];
                                    break;
                                case 6:
                                    instantiateCongratulations.transform.FindChild("Canvas").transform.FindChild("Text").GetComponent<Text>().text = wordDB.languageInfoWord6[RefreshQuizSelectionLevels.levelSelected];
                                    break;
                                case 7:
                                    instantiateCongratulations.transform.FindChild("Canvas").transform.FindChild("Text").GetComponent<Text>().text = wordDB.languageInfoWord7[RefreshQuizSelectionLevels.levelSelected];
                                    break;
                                case 8:
                                    instantiateCongratulations.transform.FindChild("Canvas").transform.FindChild("Text").GetComponent<Text>().text = wordDB.languageInfoWord8[RefreshQuizSelectionLevels.levelSelected];
                                    break;
                                case 9:
                                    instantiateCongratulations.transform.FindChild("Canvas").transform.FindChild("Text").GetComponent<Text>().text = wordDB.languageInfoWord9[RefreshQuizSelectionLevels.levelSelected];
                                    break;
                                case 10:
                                    instantiateCongratulations.transform.FindChild("Canvas").transform.FindChild("Text").GetComponent<Text>().text = wordDB.languageInfoWord10[RefreshQuizSelectionLevels.levelSelected];
                                    break;
                                case 11:
                                    instantiateCongratulations.transform.FindChild("Canvas").transform.FindChild("Text").GetComponent<Text>().text = wordDB.languageInfoWord11[RefreshQuizSelectionLevels.levelSelected];
                                    break;
                                case 12:
                                    instantiateCongratulations.transform.FindChild("Canvas").transform.FindChild("Text").GetComponent<Text>().text = wordDB.languageInfoWord12[RefreshQuizSelectionLevels.levelSelected];
                                    break;
                                case 13:
                                    instantiateCongratulations.transform.FindChild("Canvas").transform.FindChild("Text").GetComponent<Text>().text = wordDB.languageInfoWord13[RefreshQuizSelectionLevels.levelSelected];
                                    break;
                            }
                        ...
                        buttonNext.transform.FindChild("Symbol").GetComponent<SpriteRenderer>().color = new Vector4(1f, 1f, 1f, 1f);
                        ...
                        {
                            instantiateCongratulations.transform.FindChild("TextCoins").GetComponent<TextMesh>().text = "+" + 0;
                        }
